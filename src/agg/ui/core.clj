...
(defn page [{:keys [user csrf picker-config tokens members logs-enabled?]}]
  (let [csrf-headers (escape-html
    ...
    (str
     ...
     "<section id=\"video-player\" class=\"card video-player\" data-wizard-panel data-step-id=\"video-recording-clock\" hidden><div class=\"section-head\"><div><h2>Timing workspace</h2><p class=\"muted\">Set the clock range, synchronization, and optional timer on one frame-accurate timeline.</p></div></div>"
     ...
     "</div></section>"
     "<section id=\"activity-data-step\" class=\"card\" data-wizard-panel data-step-id=\"activity-data\" hidden><div class=\"section-head\"><div><h2>Choose your activity data</h2><p class=\"muted\">Heart rate is the main supported graph. Select a supported heart-rate data format, then upload a file or paste its contents. Compatible data can add optional OxiWear SpO2 (oxygen saturation).</p></div></div>"
