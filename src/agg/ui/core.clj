...
(defn page [{:keys [user csrf picker-config tokens members logs-enabled?]}]
  (let [csrf-headers (escape-html
    ...
    (str
     ...
     ".activity-file-actions{display:flex;align-items:flex-start;gap:.75rem;min-width:0;flex-wrap:wrap}.activity-file-actions .status{flex:1 1 16rem;min-width:0}.advanced-activity-data{min-width:0;margin-top:1rem;padding-top:1rem;border-top:1px solid var(--color-border)}.advanced-activity-data>div{min-width:0;margin-top:.75rem}.advanced-activity-data p{overflow-wrap:anywhere}"
     ...
     "<section class=\"card\"><div class=\"field-grid\"><label>Render preset<select id=\"preset\" required><option value=\"1080p25\">1080p · 25 fps · up to 8 minutes</option><option value=\"2.7k25\">2.7K · 25 fps · up to 4 minutes</option></select></label><label>Future trace opacity (%)<input id=\"future-trace-opacity-percent\" type=\"number\" min=\"0\" max=\"100\" step=\"any\" value=\"25\" required><small>Opacity of the not-yet-reached heart-rate trace. Default: 25%.</small></label></div></section></div>"
     ...
     "function renderWizardStep(focusHeading=false){const steps=activeWizardSteps();if(!steps.includes(wizardState.currentStep)){wizardState.currentStep=[...steps].reverse().find(stepId=>wizardState.visitedStepIds.includes(stepId))||'outcome';const state=history.state&&typeof history.state==='object'?history.state:{};history.replaceState({...state,wizardStep:wizardState.currentStep},'',location.href);}const step=wizardState.currentStep,index=steps.indexOf(step),copy=wizardStepModel.copy[step]||{title:step,description:''};wizardPanels.forEach(panel=>{panel.hidden=true;});const panel=wizardPanelForStep(step);if(panel){panel.hidden=false;panel.dataset.stepId=step;}showOnlyWizardSections(step);wizardHeading.textContent=copy.title;wizardDescription.textContent=copy.description;wizardProgress.textContent='Step '+(index+1)+' of '+steps.length;wizardOverview.hidden=!wizardState.activeRoute;wizardStepList.replaceChildren();if(wizardState.activeRoute){steps.forEach((stepId,stepIndex)=>{const item=document.createElement('li'),button=document.createElement('button'),stepCopy=wizardStepModel.copy[stepId]||{title:stepId};button.type='button';button.dataset.stepId=stepId;button.textContent=(stepIndex+1)+'. '+stepCopy.title;button.disabled=stepId!==step&&!wizardState.visitedStepIds.includes(stepId);if(stepId===step)button.setAttribute('aria-current','step');button.addEventListener('click',()=>navigateWizardStep(stepId,{push:true,focus:true}));item.append(button);wizardStepList.append(item);});}wizardBack.disabled=index<=0;wizardNext.hidden=step==='review';composeWorkflow.dataset.activeRoute=wizardState.activeRoute||'';composeWorkflow.dataset.currentStep=step;if(focusHeading)wizardHeader.focus();}"
