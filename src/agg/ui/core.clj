...
(defn page [{:keys [user csrf tokens members logs-enabled?]}]
  (let [csrf-headers (escape-html
    ...
    (str
     ...
     "<section class=\"card drive-card\"><div><h2>Google Drive</h2><p class=\"muted\">Select one video to compose telemetry over it, or leave it empty for a transparent overlay.</p></div><div class=\"drive-actions\"><a href=\"/v1/auth/drive/start\">Connect Drive</a><button id=\"open-picker\" type=\"button\">Pick one video</button><span>Selected: <output id=\"picker-selection\">None</output></span></div></section>"
     "<section class=\"card\"><div class=\"section-head\"><div><div class=\"step\">Step 2</div><h2>Optional source video</h2><p class=\"muted\">The server verifies the selected Drive file. The browser sends only its ID.</p></div></div><input id=\"source-video-file-id\" type=\"hidden\"><div class=\"field-grid\"><label>Output<select id=\"output-format\"><option value=\"h264-mp4\">H.264 MP4</option><option value=\"prores-422-mov\">ProRes 422 MOV</option></select></label><label>Fit<select id=\"fit-mode\"><option value=\"letterbox\">Letterbox / pillarbox</option><option value=\"crop\">Crop to fill</option></select></label><label>Audio<select id=\"audio-mode\"><option value=\"source+heartbeat\">Source + heartbeat</option><option value=\"source-only\">Source only</option><option value=\"heartbeat-only\">Heartbeat only</option></select></label></div></section>"
