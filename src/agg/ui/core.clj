    ...
    [{:fragment "file-sources"
     ...
     {:fragment "video-timezone"
      ...
      (str "<p>Choose the IANA timezone where the video was recorded, such as "
           ...
           "Alpha Compose asks you to confirm the timezone. The confirmed video clock "
           "controls timeline labels and the clock shown in the finished output. You "
           "can always correct detected values. Alpha Compose never treats the Google "
           "Drive upload time as the recording time.</p>")}
     {:fragment "synchronizing-data-and-camera"
      ...
      (str "<p>Choose <strong>Camera and activity devices used the same clock</strong> "
           ...
           "<p>For different clocks, drag the marker on the full video timeline to the "
           "recognizable moment and enter the activity-device time for that same moment. "
           ...
           "or End moves to a source bound. You can also type or correct either time.</p>")}
...
(defn- picker-script [picker-config]
  (let [config (when picker-config
    ...
    (str
     ...
     "acceptWizardSourceSelection();inspectRecordingClock(file);loadDrivePlayback(file);reportDiagnostic('selected','drive','selected');picker.setVisible(false);invalidatePreview();syncRequest(false);}"
...
(defn page [{:keys [user csrf picker-config tokens members logs-enabled?]}]
  (let [csrf-headers (escape-html
    ...
    (str
     ...
     ".activity-file-actions{display:flex;align-items:flex-start;gap:.75rem;min-width:0;flex-wrap:wrap}.activity-file-actions .status{flex:1 1 16rem;min-width:0}.advanced-activity-data,.advanced-output-settings{min-width:0;margin-top:1rem;padding-top:1rem;border-top:1px solid var(--color-border)}.advanced-activity-data>div,.advanced-output-settings>div{min-width:0;margin-top:.75rem}.advanced-activity-data p{overflow-wrap:anywhere}.review-sections{display:grid;gap:.75rem;min-width:0}.review-section{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:.75rem;align-items:start;min-width:0;padding:.9rem;border:1px solid var(--color-border);border-radius:.75rem;background:var(--color-surface-soft)}.review-section h3,.review-section p{margin:0}.review-section p{margin-top:.25rem;color:var(--color-muted);overflow-wrap:anywhere}.review-section button{align-self:center}"
     ".video-chrome,.video-controls-dock{min-width:0}.timing-dock{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.75rem;min-width:0;margin-top:.75rem;padding-top:.75rem;border-top:1px solid var(--color-border)}.timing-dock>*{min-width:0}.timing-dock>.full,.timing-dock>.status{grid-column:1/-1}.video-chrome:fullscreen,.video-chrome.is-fullscreen{display:grid;grid-template-rows:minmax(0,1fr) auto;width:100%;height:100dvh;max-width:none;min-width:0;overflow:hidden;padding:0;background:#010813;color:var(--color-text)}.video-chrome:fullscreen .video-stage,.video-chrome.is-fullscreen .video-stage{width:100%;height:100%;min-width:0;min-height:0;aspect-ratio:auto;border:0;border-radius:0}.video-chrome:fullscreen .video-controls-dock,.video-chrome.is-fullscreen .video-controls-dock{position:relative;z-index:2;min-width:0;max-height:58dvh;overflow-y:auto;overflow-x:hidden;padding:.8rem clamp(.6rem,2vw,1.25rem) .45rem;border-top:1px solid var(--color-border-strong);background:#031225}.video-chrome:fullscreen .video-transport,.video-chrome.is-fullscreen .video-transport{margin-top:0}.video-chrome:fullscreen .video-timeline-wrap,.video-chrome.is-fullscreen .video-timeline-wrap{margin-top:.7rem}@media(prefers-reduced-motion:reduce){.video-shortcut{transition:none}}"
     ...
     "<section id=\"video-recording-clock-step\" class=\"card\" data-wizard-panel data-step-id=\"video-recording-clock\" hidden>"
     "<section id=\"video-clock-confirmation\" class=\"source-clock\" data-confirmed=\"false\" hidden><h3>Confirm the video recording clock</h3><p class=\"muted\">Container metadata is only a suggestion. Check or correct the start, choose the IANA Video timezone, then confirm. A numeric offset alone is not a timezone.</p><p id=\"video-clock-inspection-status\" class=\"status\" role=\"status\">Choose a video to inspect its recording clock.</p><div id=\"video-clock-candidates\" class=\"clock-candidates\" role=\"radiogroup\" aria-label=\"Detected recording-clock candidates\"></div><div class=\"field-grid\"><label>Video recording start<input id=\"video-recording-start\" type=\"datetime-local\" step=\".001\"></label><div><div class=\"help-label\"><label for=\"video-timezone\">Video timezone</label>"
     ...
     "<section id=\"output-timing-step\" class=\"card\" data-wizard-panel data-step-id=\"overlay-timespan\" hidden><div class=\"section-head\"><div><h2>Set output timing</h2><p class=\"muted\">Choose the output range and how the camera and activity clocks relate. Without a source video, a valid whole-frame range reveals the clock timeline. Heart-rate data timestamps stay as provided by the file.</p></div></div>"
     ...
     "<fieldset id=\"synchronization-options\" class=\"synchronization-options full\" data-step-id=\"synchronization\"><legend><span class=\"help-label\"><strong>Camera and activity clock relationship</strong>"
     ...
     "<div id=\"manual-synchronization-fields\" class=\"field-grid full\" data-step-id=\"matching-moment\" hidden><label id=\"camera-sync-field\" class=\"sync-field\">Video time at matching moment<input id=\"camera-sync-at\" type=\"datetime-local\" step=\"0.04\"><small class=\"field-hint\">Move the marker on the full video timeline or type the video instant.</small></label>"
     "<label id=\"telemetry-sync-field\" class=\"sync-field\">Activity-device time at matching moment<input id=\"telemetry-sync-at\" type=\"datetime-local\" step=\"0.04\"><small class=\"field-hint\">Enter the activity-device instant at the same recognizable moment.</small></label></div>"
     ...
     "</div><p id=\"telemetry-file-guidance\" class=\"muted\">Upload a FIT or CSV file. Alpha Compose detects Garmin FIT or Polar CSV automatically.</p></div></div><div class=\"source-box\"><input id=\"telemetry-format\" type=\"hidden\"><input id=\"telemetry\" type=\"hidden\"><label for=\"telemetry-file\">Heart-rate file <small>Read locally in this browser and kept in memory only.</small></label><input id=\"telemetry-file\" type=\"file\" accept=\".fit,.csv,application/octet-stream,text/csv\" aria-describedby=\"telemetry-file-guidance telemetry-status\"><div class=\"activity-file-actions\"><p id=\"telemetry-status\" class=\"status\" role=\"status\"></p><button id=\"open-advanced-activity-data\" type=\"button\" aria-controls=\"advanced-activity-data\" hidden>Use Advanced activity data</button></div>"
     "<details id=\"advanced-activity-data\" class=\"advanced-activity-data\"><summary>Advanced activity data</summary><div><p class=\"muted\">Use this route for an OxiWear heart-rate CSV. OxiWear oxygen-saturation data remains an optional overlay.</p><label for=\"advanced-telemetry-format\">Heart-rate data format<select id=\"advanced-telemetry-format\" required><option value=\"\">Choose a format</option><option value=\"oxiwear-hr-csv\">OxiWear heart-rate CSV</option></select></label><label for=\"advanced-telemetry-file\" style=\"margin-top:1rem\">Heart-rate file <small>Must contain reading_time and pulse_rate columns.</small></label><input id=\"advanced-telemetry-file\" type=\"file\" accept=\".csv,text/csv\" aria-describedby=\"advanced-telemetry-status\"><p id=\"advanced-telemetry-status\" class=\"status\" role=\"status\"></p></div></details></div></section>"
     ...
     "const sourceOutputControls=byId('source-output-controls'),noSourceOutputSummary=byId('no-source-output-summary'),videoStage=byId('video-stage'),videoTransport=document.querySelector('.video-transport'),videoAudioNote=byId('video-audio-note'),noSourceRangeStatus=byId('no-source-range-status');function hasSourceVideo(){return wizardState.activeRoute!=='transparent-overlay'&&!!value('source-video-file-id');}function setComposeSourceMode(source){sourceOutputControls.hidden=!source;noSourceOutputSummary.hidden=source;videoClockConfirmation.hidden=!source;videoStage.hidden=!source;videoTransport.hidden=!source;videoStatus.hidden=!source;videoAudioNote.hidden=!source;noSourceRangeStatus.hidden=source;outputStartHandle.hidden=!source;outputEndHandle.hidden=!source;videoPlayhead.hidden=!source;videoTimeline.setAttribute('aria-label',source?'Video clock timeline':'Output clock timeline');if(source){videoChrome.hidden=false;videoTimelineWrap.hidden=false;}}"
     ...
     "function updateVideoTicks(){videoTicks.replaceChildren();const duration=outputTimelineDuration();if(!(duration>0))return;for(let index=0;index<=4;index++){const tick=document.createElement('span'),ratio=index/4;tick.textContent=videoClockText(duration*ratio,false);tick.style.left=(ratio*100)+'%';tick.style.transform=index===0?'none':index===4?'translateX(-100%)':'translateX(-50%)';videoTicks.append(tick);}updateOutputRange();}"
     ...
     "function markVideoClockUnconfirmed(message='Confirm the video clock before preview or creation.'){videoClockConfirmed=false;show(videoClockStatus,message);updateVideoSourceSummary();updateVideoTicks();updateVideoTransport();}"
     ...
     "function outputSecondsAt(clientX){const rect=videoTimeline.getBoundingClientRect();return (clientX-rect.left)/Math.max(1,rect.width)*outputTimelineDuration();}function beginRangeDrag(kind,event){if(!videoClockConfirmed)return;event.preventDefault();event.stopPropagation();rangeDragging=kind;try{event.currentTarget.setPointerCapture(event.pointerId);}catch(_error){}applyOutputBoundary(kind,outputSecondsAt(event.clientX));}function continueRangeDrag(kind,event){if(rangeDragging!==kind)return;event.preventDefault();event.stopPropagation();applyOutputBoundary(kind,outputSecondsAt(event.clientX));}function endRangeDrag(event){if(!rangeDragging)return;event.preventDefault();event.stopPropagation();rangeDragging=null;try{event.currentTarget.releasePointerCapture(event.pointerId);}catch(_error){}}function handleOutputKey(kind,event){const current=kind==='start'?outputStartSeconds:outputEndSeconds,amount=(event.shiftKey?10:1)*outputFrameSeconds;if(event.key==='ArrowLeft')applyOutputBoundary(kind,current-amount);else if(event.key==='ArrowRight')applyOutputBoundary(kind,current+amount);else if(event.key==='Home')applyOutputBoundary(kind,0);else if(event.key==='End')applyOutputBoundary(kind,outputTimelineDuration());else return;event.preventDefault();event.stopPropagation();}[[outputStartHandle,'start'],[outputEndHandle,'end']].forEach(([handle,kind])=>{handle.addEventListener('pointerdown',event=>beginRangeDrag(kind,event));handle.addEventListener('pointermove',event=>continueRangeDrag(kind,event));handle.addEventListener('pointerup',endRangeDrag);handle.addEventListener('pointercancel',endRangeDrag);handle.addEventListener('keydown',event=>handleOutputKey(kind,event));});"
     "function manualSyncLimit(){return Math.max(0,snapOutputFrame(outputTimelineDuration(),'floor'));}function setManualSyncHighlights(active){cameraSyncField.classList.toggle('sync-field-active',active);telemetrySyncField.classList.toggle('sync-field-related',active);}function updateManualSyncMarker(){const manual=selectedSynchronizationMode()==='manual-anchor';manualSyncMarker.hidden=!manual;manualSyncHelp.hidden=!manual;if(!manual){setManualSyncHighlights(false);return;}const limit=manualSyncLimit(),fieldSeconds=sourceSecondsForField('camera-sync-at'),valid=videoClockConfirmed&&Number.isFinite(fieldSeconds)&&fieldSeconds>=0&&fieldSeconds<=limit+.0001;if(valid)manualSyncSeconds=snapOutputFrame(fieldSeconds);else manualSyncSeconds=Math.min(limit,Math.max(0,manualSyncSeconds));manualSyncMarker.disabled=!valid;manualSyncMarker.style.left=(outputTimelineDuration()>0?manualSyncSeconds/outputTimelineDuration()*100:0)+'%';manualSyncMarker.setAttribute('aria-valuemax',String(limit));manualSyncMarker.setAttribute('aria-valuenow',String(manualSyncSeconds));manualSyncMarker.setAttribute('aria-valuetext',videoClockText(manualSyncSeconds));}const updateOutputRangeFields=updateOutputRange;updateOutputRange=function(){const result=updateOutputRangeFields();updateManualSyncMarker();updateTimerMarkers();return result;};function applyManualSync(proposed){const limit=manualSyncLimit();manualSyncSeconds=Math.min(limit,Math.max(0,snapOutputFrame(proposed)));setOutputField('camera-sync-at',manualSyncSeconds);updateManualSyncMarker();show(videoRangeStatus,'Manual synchronization marker snaps to 25 fps frames.','success');invalidatePreview();syncRequest(false);}function beginManualSyncDrag(event){if(manualSyncMarker.hidden||manualSyncMarker.disabled)return;event.preventDefault();event.stopPropagation();manualSyncDragging=true;setManualSyncHighlights(true);try{manualSyncMarker.setPointerCapture(event.pointerId);}catch(_error){}applyManualSync(outputSecondsAt(event.clientX));}function continueManualSyncDrag(event){if(!manualSyncDragging)return;event.preventDefault();event.stopPropagation();applyManualSync(outputSecondsAt(event.clientX));}function endManualSyncDrag(event){if(!manualSyncDragging)return;event.preventDefault();event.stopPropagation();manualSyncDragging=false;try{manualSyncMarker.releasePointerCapture(event.pointerId);}catch(_error){}setManualSyncHighlights(document.activeElement===manualSyncMarker);}function handleManualSyncKey(event){const amount=(event.shiftKey?10:1)*outputFrameSeconds;if(event.key==='ArrowLeft')applyManualSync(manualSyncSeconds-amount);else if(event.key==='ArrowRight')applyManualSync(manualSyncSeconds+amount);else if(event.key==='Home')applyManualSync(0);else if(event.key==='End')applyManualSync(manualSyncLimit());else return;event.preventDefault();event.stopPropagation();}manualSyncMarker.addEventListener('pointerdown',beginManualSyncDrag);manualSyncMarker.addEventListener('pointermove',continueManualSyncDrag);manualSyncMarker.addEventListener('pointerup',endManualSyncDrag);manualSyncMarker.addEventListener('pointercancel',endManualSyncDrag);manualSyncMarker.addEventListener('keydown',handleManualSyncKey);manualSyncMarker.addEventListener('focus',()=>setManualSyncHighlights(true));manualSyncMarker.addEventListener('blur',()=>{if(!manualSyncDragging)setManualSyncHighlights(false);});byId('camera-sync-at').addEventListener('input',updateManualSyncMarker);"
     "const updateManualSyncMarkerPosition=updateManualSyncMarker;updateManualSyncMarker=function(){const result=updateManualSyncMarkerPosition();if(selectedSynchronizationMode()==='manual-anchor')manualSyncMarker.disabled=!videoClockConfirmed;return result;};byId('camera-sync-at').addEventListener('input',updateManualSyncMarker);"
     ...
     "function syncRequest(showError){try{const request=buildRequest(),text=writeRequest(request);captureWizardState(request);show(status,'Ready to preview or create the finished video.','success');return text;}catch(error){captureWizardState(null,error.message);if(showError!==false)show(status,error.message,'error');return null;}}"
     ...
     "const telemetryFormat=byId('telemetry-format'),telemetryFile=byId('telemetry-file'),telemetryContent=byId('telemetry'),telemetryStatus=byId('telemetry-status'),advancedActivityData=byId('advanced-activity-data'),advancedTelemetryFormat=byId('advanced-telemetry-format'),advancedTelemetryFile=byId('advanced-telemetry-file'),advancedTelemetryStatus=byId('advanced-telemetry-status'),openAdvancedActivityData=byId('open-advanced-activity-data'),maxActivityFileBytes=10*1024*1024;let telemetryReadGeneration=0;"
     ...
     "function beginActivityFileSelection(input,statusNode){const generation=++telemetryReadGeneration;clearTelemetryValue();invalidatePreview();if(input===telemetryFile)advancedTelemetryFile.value='';else telemetryFile.value='';openAdvancedActivityData.hidden=true;show(telemetryStatus,'');show(advancedTelemetryStatus,'');const file=input.files&&input.files[0];if(!file){syncRequest(false);return null;}if(file.size>maxActivityFileBytes){show(statusNode,'Heart-rate files must be 10 MiB or smaller. Export a smaller individual activity file.','error');syncRequest(false);return null;}show(statusNode,'Reading the heart-rate file…');return {generation,file,statusNode};}function rejectActivityFile(selection,message,offerAdvanced=false){if(selection.generation!==telemetryReadGeneration)return;clearTelemetryValue();openAdvancedActivityData.hidden=!offerAdvanced;show(selection.statusNode,message,'error');syncRequest(false);}function acceptActivityFile(selection,format,content,label){if(selection.generation!==telemetryReadGeneration)return;telemetryFormat.value=format;setFileBackedValue('telemetry',content);openAdvancedActivityData.hidden=true;show(selection.statusNode,label,'success');syncRequest(false);}"
     ...
     "telemetryFile.addEventListener('change',()=>{const selection=beginActivityFileSelection(telemetryFile,telemetryStatus);if(!selection)return;const extension=activityFileExtension(selection.file);if(!['.fit','.csv'].includes(extension)){rejectActivityFile(selection,'Choose a .fit or .csv file exported from one individual activity.');return;}if(extension==='.fit'){readActivityFile(selection,true,result=>{if(!compatibleGarminFit(result))throw new Error('This .fit file is not a compatible Garmin FIT activity. In Garmin Connect, choose Export Original for one activity.');acceptActivityFile(selection,'garmin-fit',bytesToBase64(result),'Detected Garmin FIT.');});return;}readActivityFile(selection,false,result=>{const format=inspectActivityCsv(String(result));if(format==='oxiwear-hr-csv'){rejectActivityFile(selection,'OxiWear heart-rate CSV detected. Use Advanced activity data to add it explicitly.',true);return;}acceptActivityFile(selection,'polar-csv',String(result),'Detected Polar CSV.');});});"
     "advancedTelemetryFile.addEventListener('change',()=>{const selection=beginActivityFileSelection(advancedTelemetryFile,advancedTelemetryStatus);if(!selection)return;if(advancedTelemetryFormat.value!=='oxiwear-hr-csv'){rejectActivityFile(selection,'Choose OxiWear heart-rate CSV as the Advanced activity-data format.');return;}if(activityFileExtension(selection.file)!=='.csv'){rejectActivityFile(selection,'Choose an OxiWear .csv heart-rate file.');return;}readActivityFile(selection,false,result=>{const format=inspectActivityCsv(String(result));if(format!=='oxiwear-hr-csv')throw new Error('This is not an OxiWear heart-rate CSV with reading_time and pulse_rate columns.');acceptActivityFile(selection,advancedTelemetryFormat.value,String(result),'Loaded OxiWear heart-rate CSV.');});});openAdvancedActivityData.addEventListener('click',()=>{advancedActivityData.open=true;advancedTelemetryFormat.focus();});advancedTelemetryFormat.addEventListener('change',()=>{telemetryReadGeneration++;clearTelemetryValue();invalidatePreview();telemetryFile.value='';advancedTelemetryFile.value='';show(telemetryStatus,'');show(advancedTelemetryStatus,'');syncRequest(false);});"
     "function updateTelemetryAccept(){advancedTelemetryFile.value='';openAdvancedActivityData.hidden=true;const advanced=telemetryFormat.value==='oxiwear-hr-csv';advancedActivityData.open=advanced;advancedTelemetryFormat.value=advanced?'oxiwear-hr-csv':'';if(advanced){show(telemetryStatus,'');show(advancedTelemetryStatus,telemetryContent.value?'Loaded OxiWear heart-rate data from JSON.':'');}else show(advancedTelemetryStatus,'');}"
     ...
     "function validateWizardStep(step){if(step==='outcome'){if(!wizardState.activeRoute)throw new Error('Choose an output before continuing.');return;}if(step==='source-video'){if(!value('source-video-file-id'))throw new Error('Choose a source video before continuing.');return;}if(step==='video-recording-clock'){if(!videoClockConfirmed||!videoRecordingStartAt)throw new Error('Confirm the video recording clock before continuing.');return;}if(step==='overlay-timespan'){noSourceRange();return;}if(step==='activity-data'){required('telemetry-format','Heart-rate data format');required('telemetry','Heart-rate data');return;}if(step==='synchronization'){if(!selectedSynchronizationMode())throw new Error('Choose whether the camera and activity devices used the same clock or different clocks.');return;}if(step==='matching-moment'){localTextToIso(value('camera-sync-at'),activeZone(),'Video time at matching moment');localTextToIso(value('telemetry-sync-at'),activeZone(),'Activity-device time at matching moment');return;}if(step==='timer-overlay'){localTextToIso(value('timer-start-at'),activeZone(),'Timer start');localTextToIso(value('timer-end-at'),activeZone(),'Timer end');return;}if(step==='spo2-overlay'){required('spo2-telemetry','Oxygen-saturation data (SpO2)');return;}if(step==='watermark-overlay'){required('watermark-content','Watermark file');return;}if(step==='output-settings'){required('preset','Render preset');boundedNumber('future-trace-opacity-percent','Future trace opacity',0,100);return;}if(step==='review')buildRequest();}"
