...
(defn page [{:keys [user csrf picker-config tokens members logs-enabled?]}]
  (let [csrf-headers (escape-html
    ...
    (str
     ...
     "function playbackMimeType(evidence){return evidence?.container?.format==='mov'?'video/quicktime':evidence?.container?.format==='mp4'?'video/mp4':'';}function playbackEvidenceText(evidence){const parts=[evidence?.container?.format&&evidence.container.format.toUpperCase(),evidence?.video?.codec&&evidence.video.codec.toUpperCase(),evidence?.video?.codecTag,evidence?.audio?.codec&&evidence.audio.codec.toUpperCase()].filter(Boolean);return parts.length?' ('+parts.join(', ')+')':'';}async function browserPlaybackSupport(evidence){const mimeType=playbackMimeType(evidence),codec=evidence?.video?.codecTag;if(!mimeType||!codec)return {supported:false,reason:'the source container or video codec could not be identified'};const canPlay=sourceVideo.canPlayType(mimeType+'; codecs=\"'+codec+'\"')!=='';if(!globalThis.VideoDecoder?.isConfigSupported)return {supported:canPlay,reason:canPlay?'':'this browser does not support the selected container and codec'};try{const webCodecs=await globalThis.VideoDecoder.isConfigSupported({codec});return {supported:!!webCodecs?.supported&&canPlay,reason:!webCodecs?.supported?'WebCodecs cannot decode the selected codec':!canPlay?'the media element cannot play the selected container and codec':''};}catch(_error){return {supported:false,reason:'WebCodecs could not verify the selected codec'};}}function showUnsupportedPlayback(evidence,reason){setVideoControls(false);videoStage.hidden=true;show(videoStatus,'This video cannot play in this browser because '+reason+playbackEvidenceText(evidence)+'. It remains selected for rendering.','error');}async function loadDrivePlayback(file){const generation=++playbackGeneration;setComposeSourceMode(true);resetVideoPlayback();updateOutputFraming();show(videoStatus,'Checking selected video playback…');try{const analysisResponse=await fetch('/v1/drive/playback-analyses',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json','X-CSRF-Token':playbackCsrf},body:JSON.stringify({fileId:file.id})});if(generation!==playbackGeneration)return;if(!analysisResponse.ok)throw new Error('Playback analysis failed');const analysis=await analysisResponse.json(),support=await browserPlaybackSupport(analysis?.evidence);if(generation!==playbackGeneration)return;if(!support.supported){showUnsupportedPlayback(analysis?.evidence,support.reason);return;}const response=await fetch('/v1/drive/playback-sessions',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json','X-CSRF-Token':playbackCsrf},body:JSON.stringify({fileId:file.id})});if(generation!==playbackGeneration)return;if(!response.ok)throw new Error('Playback session failed');const session=await response.json();if(generation!==playbackGeneration)return;if(!session||typeof session.playbackUrl!=='string'||!session.playbackUrl.startsWith('/v1/drive/playback/'))throw new Error('Invalid playback session');videoStage.hidden=false;sourceVideo.src=session.playbackUrl;sourceVideo.load();}catch(_error){if(generation===playbackGeneration)show(videoStatus,'The selected video remains selected for rendering, but playback could not be prepared. Try again or choose another video.','error');}}"
