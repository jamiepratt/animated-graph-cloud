...
(defn page [{:keys [user csrf picker-config tokens members logs-enabled?]}]
  (let [csrf-headers (escape-html
    ...
    (str
     ...
     ".video-timeline{height:5.75rem}.timer-marker{position:absolute;z-index:8;top:1.5rem;width:1.35rem;min-width:1.35rem;height:2rem;margin:0;padding:0;border:2px solid #f4f9ff;border-radius:.4rem;box-shadow:0 0 0 .18rem #031225;color:#031225;font:900 .75rem ui-monospace,SFMono-Regular,Menlo,monospace;touch-action:none}.timer-marker[hidden]{display:none}.timer-marker-start{transform:translateX(-100%);background:#65e1ad}.timer-marker-end{transform:translateX(0);background:#ff8294}.timer-marker:focus{outline:3px solid var(--color-warning)}.timer-field{min-width:0;padding:.55rem;border:2px solid transparent;border-radius:.75rem}.timer-field-active{border-color:var(--color-warning);background:#ffd27a17}.timer-field-active input{outline:4px solid var(--color-warning);border-color:var(--color-warning)}"
     ...
     "<label>Section start<input id=\"section-start-at\" type=\"datetime-local\" step=\".04\" required></label>"
     "<label>Section end<input id=\"section-end-at\" type=\"datetime-local\" step=\".04\" required></label>"
