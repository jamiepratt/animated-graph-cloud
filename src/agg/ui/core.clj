...
(defn page [{:keys [user csrf picker-config tokens members logs-enabled?]}]
  (let [csrf-headers (escape-html
    ...
    (str
     ...
     "<section id=\"video-player\" class=\"card video-player\" hidden><div class=\"section-head\"><div><h2>Output framing preview</h2><p class=\"muted\">Preview the selected original inside the output frame. This is not a final encoded or activity-data overlay preview.</p></div></div><div id=\"video-chrome\" class=\"video-chrome\"><div id=\"video-stage\" class=\"video-stage\"><video id=\"source-video-player\" playsinline preload=\"metadata\"></video></div>"
