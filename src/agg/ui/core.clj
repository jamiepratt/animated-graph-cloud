...
(defn preview-operation-fragment
  [{:keys [id state progressPercent error result receiptExpiresAt]} generation]
      ...
      (if (seq (:sections result))
        (let [assets (into {} (map (juxt :id identity)) (:assets result))
          ...
          (str "<article id=\"preview-result\" class=\"preview-gallery\" data-preview-operation=\""
               (escape-html id) "\" data-preview-receipt-expires-at=\""
               (escape-html receiptExpiresAt) "\" data-preview-generation=\""
...
(defn preview-gate-fragment [{:keys [error requestId retryable]}]
  (let [[title message]
        (case error
          "preview_pending"
          ["Preview still running" "Wait for Preview to finish before submitting."]
          "preview_expired"
          ["Preview expired" "Run Preview again before submitting."]
          "preview_stale"
          ["Preview no longer matches" "Run Preview again for the current settings."]
          ["Preview required" "Run Preview successfully before submitting."])]
    (str "<article class=\"notice preview-submit-blocked\" role=\"alert\" data-preview-gate=\""
         (escape-html error) "\"><h2>" title "</h2><p>" message
         " No durable render was submitted or charged.</p><dl><dt>Request ID</dt><dd><code>"
         (escape-html requestId) "</code></dd><dt>Retryable</dt><dd>"
         (if retryable "Yes" "No") "</dd></dl></article>")))
...
(defn page [{:keys [user csrf picker-config tokens members logs-enabled?]}]
  (let [csrf-headers (escape-html
    ...
    (str
     ...
     ".muted,.hint{color:#5c6b82}.eyebrow{color:#4374c5;font-size:.75rem;font-weight:800;letter-spacing:.12em;text-transform:uppercase}.session-controls{display:flex;align-items:flex-end;flex-direction:column;gap:.65rem}.session-controls p,.session-controls form{margin:0}"
     ".card{background:white;border:1px solid #e1e7f0;border-radius:1.1rem;box-shadow:0 1rem 3rem #243b5a0d;padding:1.35rem;margin:1rem 0}"
     ...
     "<input id=\"render-request\" type=\"hidden\" name=\"request\" value=\"{}\"><input id=\"preview-operation-id\" type=\"hidden\" name=\"previewOperationId\" value=\"\">"
     ...
     "<section class=\"card\"><div class=\"actions\"><button id=\"preview-button\" class=\"primary\" type=\"button\" hx-post=\"/ui/preview\" hx-include=\"closest form\" hx-target=\"#preview-result\" hx-swap=\"outerHTML\" hx-request='{\"timeout\":15000}'>Preview overlay</button><button id=\"submit-button\" class=\"primary\" type=\"submit\" disabled aria-disabled=\"true\">Submit render</button><span id=\"preview-submit-status\" class=\"status\" role=\"status\">Preview required before Submit.</span><span id=\"form-status\" class=\"status\" role=\"status\"></span></div></section></form>"
     ...
     "const status=document.getElementById('form-status'), jsonStatus=document.getElementById('json-status'),submitButton=document.getElementById('submit-button'),previewReceipt=document.getElementById('preview-operation-id'),submitStatus=document.getElementById('preview-submit-status');"
     ...
     "const previewButton=byId('preview-button');function newPreviewGeneration(){return crypto.randomUUID?crypto.randomUUID():String(Date.now())+'-'+Math.random();}let previewGeneration=newPreviewGeneration(),previewActive=false,submitActive=false,receiptExpiryTimer=null;function setPreviewActive(active){previewActive=active;previewButton.disabled=active;previewButton.setAttribute('aria-disabled',String(active));}function setSubmitGate(state,operationId='',expiresAt=''){if(receiptExpiryTimer){clearTimeout(receiptExpiryTimer);receiptExpiryTimer=null;}const ready=state==='succeeded'&&!!operationId&&Date.parse(expiresAt)>Date.now();previewReceipt.value=ready?operationId:'';submitButton.disabled=!ready||submitActive;submitButton.setAttribute('aria-disabled',String(submitButton.disabled));const messages={required:'Preview required before Submit.',pending:'Preview is still running. Submit is disabled.',succeeded:'Preview succeeded. Submit is enabled.',failed:'Preview failed. Run Preview again.',stale:'Preview settings changed. Run Preview again.',expired:'Preview expired. Run Preview again.'};show(submitStatus,messages[ready?'succeeded':(state==='succeeded'?'expired':state)]||messages.required,ready?'success':(state==='pending'?'':'error'));if(ready){const delay=Math.max(0,Date.parse(expiresAt)-Date.now());receiptExpiryTimer=setTimeout(()=>setSubmitGate('expired'),Math.min(delay,2147483647));}}function localPreviewState(className,title,message){const result=byId('preview-result');result.className=className;result.setAttribute('role',className==='preview-pending'?'status':'alert');result.dataset.previewGeneration=previewGeneration;delete result.dataset.previewOperation;delete result.dataset.previewReceiptExpiresAt;result.replaceChildren();const heading=document.createElement('h2'),copy=document.createElement('p');heading.textContent=title;copy.textContent=message;result.append(heading,copy);return result;}function beginPreview(){previewGeneration=newPreviewGeneration();submitActive=false;setSubmitGate('pending');setPreviewActive(true);localPreviewState('preview-pending','Preparing preview','Waiting for the preview service.');show(status,'Preparing preview…');}function finishPreview(){setPreviewActive(false);}function invalidatePreview(){const hadPreview=previewActive||!!previewReceipt.value||!!byId('preview-result')?.dataset?.previewOperation;previewGeneration=newPreviewGeneration();submitActive=false;setSubmitGate(hadPreview?'stale':'required');finishPreview();if(hadPreview){localPreviewState('preview-stale','Preview settings changed','Start a new preview for the current settings.');show(status,'Preview settings changed.','');}}"
     "function previewTrigger(event){const trigger=event.detail?.elt||event.target;return !!trigger?.matches?.('[hx-post=\"/ui/preview\"]');}function previewTarget(event){const target=event.target?.id==='preview-result'?event.target:event.detail?.target;return target?.id==='preview-result'?target:null;}function previewRequestEvent(event){const trigger=event.detail?.elt||event.target;return previewTrigger(event)||trigger?.id==='preview-result'||!!trigger?.closest?.('#preview-result')||!!previewTarget(event);}function requestPreviewGeneration(event){const trigger=event.detail?.elt||event.target;return event.detail?.requestConfig?.headers?.['X-Preview-Generation']||trigger?.dataset?.previewGeneration;}function previewEventGeneration(event){return event.detail?.xhr?.aggPreviewGeneration||event.detail?.requestConfig?.headers?.['X-Preview-Generation']||event.detail?.xhr?.getResponseHeader?.('X-Preview-Generation')||requestPreviewGeneration(event)||previewTarget(event)?.dataset?.previewGeneration||previewGeneration;}function currentPreviewEvent(event){return previewEventGeneration(event)===previewGeneration;}function transportFailure(event,kind){if(!previewRequestEvent(event)||!previewActive||!currentPreviewEvent(event))return;const statusCode=Number(event.detail?.xhr?.status||0);finishPreview();setSubmitGate('failed');if(kind==='platform'){localPreviewState('preview-error','Preview did not finish','The preview service returned status '+statusCode+'. No durable render was submitted or charged. Retry with the Preview button.');show(status,'Preview failed. See details below.','error');}else if(kind==='connection'){localPreviewState('preview-error','Preview connection lost','The preview connection was lost. No durable render was submitted or charged. Check the connection, then retry.');show(status,'Preview connection lost.','error');}else if(kind==='cancelled'){localPreviewState('preview-error','Preview request cancelled','The browser cancelled this preview request. No durable render was submitted or charged. Retry when ready.');show(status,'Preview request cancelled.','error');}else{localPreviewState('preview-error','Preview did not finish','The browser stopped waiting for the preview service. No durable render was submitted or charged. Retry with the Preview button.');show(status,'Preview timed out.','error');}}"
     "form.addEventListener('input',event=>{if(event.target.type==='file')return;if(event.target.id==='raw-json'){invalidatePreview();return;}clearFileBackedValue(event.target.id);invalidatePreview();syncRequest(false);});document.body.addEventListener('htmx:configRequest',event=>{if(!form.contains(event.target))return;const preview=previewTrigger(event);if(preview&&previewActive){event.preventDefault();return;}if(!preview&&(submitButton.disabled||submitActive||!previewReceipt.value)){event.preventDefault();setSubmitGate(previewReceipt.value?'pending':'required');return;}const text=syncRequest(true);if(!text){event.preventDefault();return;}event.detail.parameters.request=text;if(preview){beginPreview();event.detail.headers['X-Preview-Generation']=previewGeneration;}else{submitActive=true;submitButton.disabled=true;submitButton.setAttribute('aria-disabled','true');event.detail.parameters.previewOperationId=previewReceipt.value;event.detail.headers['Idempotency-Key']='ui-preview-'+previewReceipt.value;show(submitStatus,'Submitting durable render…');}});document.body.addEventListener('htmx:beforeSend',event=>{if(previewRequestEvent(event)&&event.detail?.xhr)event.detail.xhr.aggPreviewGeneration=requestPreviewGeneration(event)||previewGeneration;});document.body.addEventListener('htmx:beforeSwap',event=>{const target=previewTarget(event);if(!target)return;const generation=previewEventGeneration(event);if(generation&&generation!==previewGeneration)event.detail.shouldSwap=false;});document.body.addEventListener('htmx:afterSwap',event=>{const target=previewTarget(event);if(!target){const swapped=event.detail?.target||event.target,gate=swapped?.querySelector?.('.preview-submit-blocked');if(gate){submitActive=false;setSubmitGate(gate.dataset.previewGate.replace('preview_',''));show(status,gate.querySelector('h2')?.textContent||'Preview required.','error');}return;}if(!currentPreviewEvent(event))return;if(target.matches('.preview-pending')){setSubmitGate('pending');setPreviewActive(true);show(status,'Preparing preview…');}else{finishPreview();if(target.matches('.preview-error')){setSubmitGate('failed');show(status,'Preview failed. See details below.','error');}else if(target.matches('.preview-gallery,.preview-empty')){setSubmitGate('succeeded',target.dataset.previewOperation,target.dataset.previewReceiptExpiresAt);show(status,target.matches('.preview-gallery')?'Preview ready.':'Preview completed with no moments.','success');}else if(target.matches('.preview-stale')){setSubmitGate('expired');show(status,'Preview expired.','error');}}});document.body.addEventListener('htmx:responseError',event=>transportFailure(event,'platform'));document.body.addEventListener('htmx:sendError',event=>transportFailure(event,'connection'));document.body.addEventListener('htmx:timeout',event=>transportFailure(event,'timeout'));document.body.addEventListener('htmx:sendAbort',event=>transportFailure(event,'cancelled'));document.body.addEventListener('htmx:xhr:abort',event=>transportFailure(event,'cancelled'));"
