...
(defn page [{:keys [user csrf tokens members logs-enabled?]}]
  (let [csrf-headers (escape-html
    ...
    (str
     ...
     "details summary{cursor:pointer;font-weight:800;color:#315b9d}.raw-panel textarea{min-height:18rem}.raw-actions{display:flex;gap:.65rem;flex-wrap:wrap;margin-top:.7rem}"
     ...
     "<div class=\"shell\"><header><div><div class=\"eyebrow\">Telemetry overlays for video</div><h1>Compose your overlay</h1><p class=\"muted\">Configure a render, preview it, then send the finished overlay to Drive.</p></div>"
     "<p class=\"muted\">Signed in as " (escape-html (:email user)) "</p></header>"
     "<section class=\"card drive-card\"><div><h2>Google Drive</h2><p class=\"muted\">Optional connection for selecting inputs and receiving completed overlays.</p></div><div class=\"drive-actions\"><a href=\"/v1/auth/drive/start\">Connect Drive</a><button id=\"open-picker\" type=\"button\">Open Picker</button><span>Selected: <output id=\"picker-selection\">None</output></span></div></section>"
     ...
     "<section class=\"card raw-panel\"><details><summary>Advanced: paste or inspect raw JSON</summary><p class=\"hint\">Paste a request and choose “Apply to form”. Form edits are reflected here before preview or submission.</p><textarea id=\"raw-json\" spellcheck=\"false\">{}</textarea><div class=\"raw-actions\"><button id=\"apply-json\" type=\"button\">Apply JSON to form</button><button id=\"copy-json\" type=\"button\">Copy generated JSON</button></div><p id=\"json-status\" class=\"status\" role=\"status\"></p></details></section>"
     ...
     "function applyRequest(request){if(!request||typeof request!=='object'||Array.isArray(request))throw new Error('JSON must be an object');['telemetryFormat','preset'].forEach(key=>{if(request[key]!==undefined)byId(key.replace(/[A-Z]/g,letter=>'-'+letter.toLowerCase())).value=request[key];});byId('timezone').value='local';[['telemetrySyncAt','telemetry-sync-at'],['cameraSyncAt','camera-sync-at'],['sectionStartAt','section-start-at'],['sectionEndAt','section-end-at']].forEach(([key,id])=>{byId(id).value=request[key]?isoToLocal(request[key]):'';});byId('telemetry').value=request.telemetry||'';byId('telemetry-file').value='';show(byId('telemetry-status'),request.telemetry?'Loaded from JSON.':'');const hasSpo2=!!request.spo2;byId('spo2-enabled').checked=hasSpo2;refreshOptional('spo2-enabled','spo2-fields');byId('spo2-telemetry').value=hasSpo2?(request.spo2.telemetry||''):'';show(byId('spo2-status'),hasSpo2?'Loaded from JSON.':'');const hasTimer=!!request.timer;byId('timer-enabled').checked=hasTimer;refreshOptional('timer-enabled','timer-fields');byId('timer-start-at').value=hasTimer?isoToLocal(request.timer.startAt):'';byId('timer-end-at').value=hasTimer?isoToLocal(request.timer.endAt):'';const hasWatermark=!!request.watermark;byId('watermark-enabled').checked=hasWatermark;refreshOptional('watermark-enabled','watermark-fields');byId('watermark-file').value='';byId('watermark-content').value=hasWatermark?(request.watermark.contentBase64||''):'';show(byId('watermark-status'),hasWatermark?'Loaded from JSON.':'');updateTelemetryAccept();writeRequest(request);show(jsonStatus,'JSON applied to the form.','success');show(status,'Ready to preview or submit.','success');}"
     "byId('apply-json').addEventListener('click',()=>{try{applyRequest(JSON.parse(raw.value));}catch(error){show(jsonStatus,error.message,'error');}});byId('copy-json').addEventListener('click',()=>{const text=syncRequest(false)||raw.value;navigator.clipboard?navigator.clipboard.writeText(text).then(()=>show(jsonStatus,'Generated JSON copied.','success')):show(jsonStatus,'Copy is unavailable in this browser.');});"
     "form.addEventListener('input',event=>{if(event.target.id!=='raw-json')syncRequest(false);});document.body.addEventListener('htmx:configRequest',event=>{if(!form.contains(event.target))return;const text=syncRequest(true);if(!text){event.preventDefault();return;}event.detail.parameters.request=text;});"
     "const selection=byId('picker-selection');byId('open-picker').addEventListener('click',()=>{window.open('/v1/drive/picker','agg-picker','popup,width=960,height=720');});window.addEventListener('message',event=>{if(event.origin!==location.origin||event.data?.type!=='agg-picker')return;selection.textContent=event.data.files.map(file=>file.name).join(', ')||'None';});"
