    ...
    [{:fragment "file-sources"
     ...
     {:fragment "synchronizing-data-and-camera"
      ...
      (str "<p>Choose <strong>Camera and activity devices used the same clock</strong> "
           ...
           "You do not need to enter an absolute camera time. "
...
(defn page [{:keys [user csrf picker-config tokens members logs-enabled?]}]
  (let [csrf-headers (escape-html
    ...
    (str
     ...
     "<section id=\"output-timing-step\" class=\"card\" data-wizard-panel data-step-id=\"overlay-timespan\" hidden><div class=\"section-head\"><div><h2>Synchronize video and activity data</h2><p class=\"muted\">Confirm a shared clock or match one source-video frame to the same instant in the activity data. Heart-rate timestamps remain the clock authority.</p></div></div>"
     ...
     "<p id=\"no-source-range-status\" class=\"status full\" role=\"status\">Enter Output start and Output end to reveal the output clock timeline.</p><div id=\"timeline-mode\" class=\"full\"><strong id=\"timeline-mode-label\">Elapsed time</strong><p id=\"timeline-mode-status\" class=\"status\" role=\"status\" aria-live=\"polite\">Timeline labels show elapsed source time.</p></div>"
     "</div></section>"
     ...
     "function manualActivityInstant(){if(!value('telemetry-sync-at'))return null;try{return localTextToIso(value('telemetry-sync-at'),activeZone(),'Activity-data time at the selected frame');}catch(_error){return null;}}function updateTimelineMode(announce=false){const manual=finishedManualSynchronization(),synced=manual&&videoClockConfirmed&&videoClockSource==='manual-anchor',zone=activeZone(),key=synced?'synced:'+zone:manual?'elapsed':videoClockConfirmed?'recording:'+zone:'elapsed',previous=timelineModeKey;timelineModeKey=key;if(synced){timelineModeLabel.textContent='Synced recording time · '+zone;if(announce&&previous!==key)show(timelineModeStatus,'Timeline labels now show synced recording time ('+zone+').','success');else if(!timelineModeStatus.textContent)show(timelineModeStatus,'Timeline labels show synced recording time ('+zone+').');}else if(manual){timelineModeLabel.textContent='Elapsed time';if(announce&&previous!==key)show(timelineModeStatus,'Timeline labels now show elapsed time.','');else if(!timelineModeStatus.textContent||previous!==key)show(timelineModeStatus,'Timeline labels show elapsed source time.','');}else{timelineModeLabel.textContent=videoClockConfirmed?'Recording time · '+zone:'Elapsed time';if(previous!==key)show(timelineModeStatus,videoClockConfirmed?'Timeline labels show recording time ('+zone+').':'Timeline labels show elapsed time.','');}}"
     ...
     "function validateWizardStep(step){if(step==='outcome'){if(!wizardState.activeRoute)throw new Error('Choose an output before continuing.');return;}if(step==='source-video'){if(!value('source-video-file-id'))throw new Error('Choose a source video before continuing.');return;}if(step==='overlay-timespan'){noSourceRange();return;}if(step==='activity-data'){required('telemetry-format','Heart-rate data format');required('telemetry','Heart-rate data');return;}if(step==='synchronization'){const mode=selectedSynchronizationMode();if(!mode)throw new Error('Choose whether the camera and activity devices used the same clock or different clocks.');if(wizardState.activeRoute==='finished-video'){if(mode==='shared-clock'&&(!videoClockConfirmed||videoClockSource!=='shared-clock'))throw new Error('Confirm the shared video recording clock before continuing.');if(mode==='manual-anchor'&&!deriveManualSynchronization(false))throw new Error('Choose a source-video frame and enter the matching activity-data time.');}return;}if(step==='matching-moment'){localTextToIso(value('camera-sync-at'),activeZone(),'Video time at matching moment');localTextToIso(value('telemetry-sync-at'),activeZone(),'Activity-device time at matching moment');return;}if(step==='timer-overlay'){localTextToIso(value('timer-start-at'),activeZone(),'Timer start');localTextToIso(value('timer-end-at'),activeZone(),'Timer end');return;}if(step==='spo2-overlay'){required('spo2-telemetry','Oxygen-saturation data (SpO2)');return;}if(step==='watermark-overlay'){required('watermark-content','Watermark file');return;}if(step==='output-settings'){required('preset','Render preset');boundedNumber('future-trace-opacity-percent','Future trace opacity',0,100);return;}if(step==='review')buildRequest();}"
