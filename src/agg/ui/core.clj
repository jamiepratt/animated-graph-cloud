...
(defn- picker-script [picker-config]
  (let [config (when picker-config
    ...
    (str
     ...
     "loadDrivePlayback(file);reportDiagnostic('selected','drive','selected');picker.setVisible(false);invalidatePreview();syncRequest(false);}"
...
(defn page [{:keys [user csrf picker-config tokens members logs-enabled?]}]
  (let [csrf-headers (escape-html
    ...
    (str
     ...
     "<section id=\"video-player\" class=\"card video-player\" hidden><div class=\"section-head\"><div><h2>Output framing preview</h2><p class=\"muted\">Preview the selected original inside the output frame. This is not a final encoded or activity-data overlay preview.</p></div></div><div id=\"video-stage\" class=\"video-stage\"><video id=\"source-video-player\" playsinline preload=\"metadata\"></video></div>"
     "<div class=\"video-transport\" aria-label=\"Video playback controls\"><output id=\"video-time\" class=\"video-time\">00:00:00.000 / 00:00:00.000</output><button type=\"button\" data-seek-seconds=\"-60\" aria-label=\"Jump back 60 seconds\">-60</button><button type=\"button\" data-seek-seconds=\"-10\" aria-label=\"Jump back 10 seconds\">-10</button><button id=\"video-play-pause\" type=\"button\" disabled>Play</button><button type=\"button\" data-seek-seconds=\"10\" aria-label=\"Jump forward 10 seconds\">+10</button><button type=\"button\" data-seek-seconds=\"60\" aria-label=\"Jump forward 60 seconds\">+60</button><label class=\"video-volume\">Volume<input id=\"video-volume\" type=\"range\" min=\"0\" max=\"1\" step=\"0.05\" value=\"1\" aria-label=\"Video volume\"></label><button id=\"video-fullscreen\" type=\"button\">Fullscreen</button></div>"
     "<div class=\"video-timeline-wrap\"><div id=\"video-timeline\" class=\"video-timeline\" role=\"slider\" tabindex=\"0\" aria-label=\"Video timeline\" aria-valuemin=\"0\" aria-valuemax=\"0\" aria-valuenow=\"0\" aria-valuetext=\"00:00:00.000\" aria-disabled=\"true\"><div class=\"video-timeline-track\"><div id=\"video-buffered-ranges\" class=\"video-buffered-ranges\"></div><div id=\"video-playhead\" class=\"video-playhead\" style=\"left:0%\"></div></div><output id=\"video-timeline-tooltip\" class=\"video-timeline-tooltip\" hidden>00:00:00.000</output></div><div id=\"video-ticks\" class=\"video-ticks\" aria-hidden=\"true\"></div></div>"
     ...
     "const playbackCsrf=" (json-script csrf) ",videoPlayer=byId('video-player'),sourceVideo=byId('source-video-player'),videoStage=byId('video-stage'),videoPlayPause=byId('video-play-pause'),videoTime=byId('video-time'),videoTimeline=byId('video-timeline'),videoPlayhead=byId('video-playhead'),videoTooltip=byId('video-timeline-tooltip'),videoTicks=byId('video-ticks'),videoBuffered=byId('video-buffered-ranges'),videoStatus=byId('video-player-status'),videoVolume=byId('video-volume');let playbackGeneration=0,videoDuration=0,videoScrubbing=false,videoLoadRetryUsed=false,videoLoadRetrySource='';"
     ...
     "function updateVideoTicks(){videoTicks.replaceChildren();if(!playableDuration())return;for(let index=0;index<=4;index++){const tick=document.createElement('span'),ratio=index/4;tick.textContent=playbackTime(videoDuration*ratio);tick.style.left=(ratio*100)+'%';tick.style.transform=index===0?'none':index===4?'translateX(-100%)':'translateX(-50%)';videoTicks.append(tick);}}"
     ...
     "function setVideoControls(enabled){videoPlayPause.disabled=!enabled;document.querySelectorAll('[data-seek-seconds]').forEach(button=>button.disabled=!enabled);byId('video-fullscreen').disabled=!enabled;videoVolume.disabled=!enabled;videoTimeline.setAttribute('aria-disabled',String(!enabled));}"
     ...
     "async function loadDrivePlayback(file){const generation=++playbackGeneration;resetVideoPlayback();videoPlayer.hidden=false;updateOutputFraming();show(videoStatus,'Loading selected video…');try{const response=await fetch('/v1/drive/playback-sessions',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json','X-CSRF-Token':playbackCsrf},body:JSON.stringify({fileId:file.id})});if(generation!==playbackGeneration)return;if(!response.ok){if(response.status===415)show(videoStatus,'This video remains selected for rendering, but its original format cannot play in this browser. Choose another video for playback.','error');else show(videoStatus,'The selected video remains selected for rendering, but playback could not be prepared. Try again or choose another video.','error');return;}const session=await response.json();if(generation!==playbackGeneration)return;if(!session||typeof session.playbackUrl!=='string'||!session.playbackUrl.startsWith('/v1/drive/playback/'))throw new Error('Invalid playback session');videoLoadRetryUsed=false;videoLoadRetrySource=session.playbackUrl;sourceVideo.src=session.playbackUrl;sourceVideo.load();}catch(_error){if(generation===playbackGeneration)show(videoStatus,'The selected video remains selected for rendering, but playback could not be prepared. Try again or choose another video.','error');}}"
     "sourceVideo.addEventListener('loadedmetadata',()=>{videoDuration=Number(sourceVideo.duration);if(!playableDuration()){show(videoStatus,'This video remains selected for rendering, but its duration is unavailable for browser playback.','error');setVideoControls(false);return;}sourceVideo.currentTime=0;videoTimeline.setAttribute('aria-valuemax',String(videoDuration));setVideoControls(true);updateVideoTicks();updateBufferedRanges();updateVideoTransport();show(videoStatus,'Ready. Click or drag the timeline to seek.','success');});sourceVideo.addEventListener('timeupdate',updateVideoTransport);sourceVideo.addEventListener('progress',updateBufferedRanges);sourceVideo.addEventListener('play',updateVideoTransport);sourceVideo.addEventListener('pause',updateVideoTransport);sourceVideo.addEventListener('error',()=>{const source=sourceVideo.getAttribute('src');if(!source)return;if(!videoLoadRetryUsed&&source===videoLoadRetrySource){videoLoadRetryUsed=true;show(videoStatus,'Retrying selected video playback…');setTimeout(()=>{if(sourceVideo.getAttribute('src')===source&&!playableDuration())sourceVideo.load();},100);return;}setVideoControls(false);show(videoStatus,'This video remains selected for rendering, but its original format or codecs cannot play in this browser. Choose another video for playback.','error');});"
     "videoPlayPause.addEventListener('click',()=>{if(sourceVideo.paused)sourceVideo.play().catch(()=>show(videoStatus,'Playback could not start. Try again.','error'));else sourceVideo.pause();});document.querySelectorAll('[data-seek-seconds]').forEach(button=>button.addEventListener('click',()=>seekVideo(sourceVideo.currentTime+Number(button.dataset.seekSeconds))));videoVolume.addEventListener('input',()=>{sourceVideo.volume=Number(videoVolume.value);});byId('video-fullscreen').addEventListener('click',()=>{if(document.fullscreenElement)document.exitFullscreen();else videoStage.requestFullscreen?.();});"
     ...
     "['output-format','fit-mode','audio-mode'].forEach(id=>byId(id).addEventListener('input',()=>{if(id==='fit-mode')updateOutputFraming();invalidatePreview();syncRequest(false);}));updateOutputFraming();setVideoControls(false);"
