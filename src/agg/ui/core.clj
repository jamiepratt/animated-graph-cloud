...
(defn- wizard-outcome-step []
  (str
   ...
   "data-step-id=\"outcome\" aria-labelledby=\"wizard-outcome-heading\">"
   "<div class=\"step\">Step 1</div>"
   "<h2 id=\"wizard-outcome-heading\">What would you like to make?</h2>"
...
(defn page [{:keys [user csrf picker-config tokens members logs-enabled?]}]
  (let [csrf-headers (escape-html
    ...
    (str
     ...
     ".visually-hidden{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}.wizard-step{min-width:0}.wizard-choices{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.8rem;min-width:0;margin:1rem 0 0;padding:0;border:0}.wizard-choice{display:flex;align-items:flex-start;gap:.7rem;min-width:0;padding:1rem;border:1px solid var(--color-border);border-radius:.8rem;background:var(--color-surface-soft);cursor:pointer}.wizard-choice:focus-within{outline:3px solid var(--color-warning);outline-offset:2px}.wizard-choice input{flex:0 0 auto;width:auto;margin:.25rem 0 0}.wizard-choice span{min-width:0}.wizard-choice strong,.wizard-choice small{display:block;overflow-wrap:anywhere}.wizard-choice small{margin-top:.25rem;color:var(--color-muted);font-weight:400}"
     ...
     "@media(max-width:680px){.shell{padding:1rem .8rem 3rem}.task-header,.drive-card,.section-head{display:block}.session-controls{align-items:flex-start;margin-top:1rem}.wizard-choices,.field-grid{grid-template-columns:1fr}.drive-actions{margin-top:1rem}.video-transport{justify-content:flex-start}.video-time{width:100%;min-width:0;text-align:left}.video-volume{flex:1 1 10rem}.video-volume input{min-width:0;width:100%}.preview-moment{flex:0 1 24rem;width:min(100%,24rem);border-top:1px solid #e1e7f0;padding:1rem 0}.preview-moment:first-child{border-top:0;padding-top:0}.preview-cell{width:100%;margin-top:.75rem}}"
     (theme-style)
     "</style>"
     ...
     "<section id=\"no-source-output-summary\" class=\"card output-summary\"><h2>Output format</h2><p><strong>Transparent ProRes 4444 overlay</strong></p><p class=\"muted\">No source video is selected. Alpha Compose will create a transparent overlay for a separate editing workflow.</p></section>"
     "<section id=\"source-output-controls\" class=\"card\" hidden><div class=\"section-head\"><div><h2>Output format</h2><p class=\"muted\">Choose the composite format and how the selected source fits the frame. The server verifies its MIME type, 2 GiB limit, download access, decodability, and duration.</p></div></div><input id=\"source-video-file-id\" type=\"hidden\"><div class=\"field-grid\"><label>Output format<select id=\"output-format\"><option value=\"h264-mp4\">H.264 MP4</option><option value=\"prores-422-mov\">ProRes 422 MOV</option></select></label><label>Fit<select id=\"fit-mode\"><option value=\"letterbox\">Letterbox / pillarbox</option><option value=\"crop\">Crop to fill</option></select></label><div><div class=\"help-label\"><label for=\"audio-mode\">Audio</label>"
     ...
     "<section id=\"video-player\" class=\"card video-player\"><div class=\"section-head\"><div><h2>Timing workspace</h2><p class=\"muted\">Set the clock range, synchronization, and optional timer on one frame-accurate timeline.</p></div></div>"
     ...
     "<div id=\"manual-synchronization-fields\" class=\"field-grid full\" hidden><label id=\"camera-sync-field\" class=\"sync-field\">Video time at matching moment<input id=\"camera-sync-at\" type=\"datetime-local\" step=\"0.04\"><small class=\"field-hint\">Move the marker on the full video timeline or type the video instant.</small></label>"
     ...
     "<label>Output start<input id=\"section-start-at\" type=\"datetime-local\" step=\".04\" required></label>"
     "<label>Output end<input id=\"section-end-at\" type=\"datetime-local\" step=\".04\" required></label>"
     ...
     "<div class=\"optional full\"><label class=\"toggle\"><input id=\"timer-enabled\" type=\"checkbox\"> Show elapsed timer</label><div id=\"timer-fields\" hidden class=\"field-grid\"><label id=\"timer-start-field\" class=\"timer-field\">Timer start<input id=\"timer-start-at\" type=\"datetime-local\" step=\".04\"></label><label id=\"timer-end-field\" class=\"timer-field\">Timer end<input id=\"timer-end-at\" type=\"datetime-local\" step=\".04\"></label></div></div></div></section>"
     "<section class=\"card\"><div class=\"section-head\"><div><h2>Choose your activity data</h2><p class=\"muted\">Heart rate is the main supported graph. Select a supported heart-rate data format, then upload a file or paste its contents. Compatible data can add optional OxiWear SpO2 (oxygen saturation).</p></div></div>"
     ...
     "<section class=\"card\"><div class=\"section-head\"><div><h2>Optional overlays</h2><p class=\"muted\">Add supporting data only when it is present in this render.</p></div></div>"
     ...
     "[['spo2-enabled','spo2-fields'],['watermark-enabled','watermark-fields']].forEach(([toggle,fields])=>byId(toggle).addEventListener('change',()=>{refreshOptional(toggle,fields);syncRequest(false);}));timerEnabled.addEventListener('change',()=>{refreshOptional('timer-enabled','timer-fields');if(timerEnabled.checked)initializeTimer();else updateTimerMarkers();syncRequest(false);});"
