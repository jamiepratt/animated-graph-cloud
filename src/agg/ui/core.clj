...
(defn page [{:keys [user csrf picker-config tokens members logs-enabled?]}]
  (let [csrf-headers (escape-html
    ...
    (str
     ...
     "<label>Section end<input id=\"section-end-at\" type=\"datetime-local\" step=\".04\" required></label></div></section>"
     ...
     "const validateRequestWithoutLegacyCameraOrder=validateRequest;validateRequest=function(request){return validateRequestWithoutLegacyCameraOrder(request).filter(error=>error!=='Request timestamps must satisfy cameraSyncAt <= sectionStartAt < sectionEndAt.');};"
