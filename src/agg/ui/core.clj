...
(defn page [{:keys [user csrf picker-config tokens members logs-enabled?]}]
  (let [csrf-headers (escape-html
    ...
    (str
     ...
     "<section id=\"confirm-video-clock-step\" class=\"card\" data-wizard-panel data-step-id=\"confirm-video-clock\" hidden><section id=\"video-clock-confirmation\" class=\"source-clock\" data-confirmed=\"false\" hidden><h3>Confirm the video recording clock</h3><p class=\"muted\">Container metadata is only a suggestion. Check or correct the start, choose the IANA Video timezone, then confirm. A numeric offset alone is not a timezone.</p><div id=\"video-clock-candidates\" class=\"clock-candidates\" role=\"radiogroup\" aria-label=\"Detected recording-clock candidates\"></div><div class=\"field-grid\"><label>Video recording start<input id=\"video-recording-start\" type=\"datetime-local\" step=\".001\"></label><div><div class=\"help-label\"><label for=\"video-timezone\">Video timezone</label>"
     ...
     "const wizardStepCopy={...wizardStepModel.copy,'source-video':{title:'Choose your source video',description:'Pick one Google Drive video, then review its detected recording-clock hint.'},'synchronization-decision':{title:'Compare the camera and activity clocks',description:'Choose whether the source video already follows the activity-device clock or whether you need to match one moment.'},'confirm-video-clock':{title:'Confirm the video recording clock',description:'Check or correct the recording start and timezone before setting the output range.'},'matching-moment':{title:'Match one moment',description:'Move to one recognizable source-video frame and enter the same instant from the activity data.'},'output-timespan':{title:'Choose the output timespan',description:'Set the display timezone plus the start and end instants for the output range.'}};"
     ...
     "function activeWizardSteps(){if(!wizardState.activeRoute)return ['outcome'];const steps=wizardState.activeRoute==='transparent-overlay'?['outcome','activity-data','output-timespan']:['outcome','source-video','activity-data','synchronization-decision'];if(wizardState.activeRoute==='finished-video'){const mode=activeSynchronizationMode();if(mode==='manual-anchor')steps.push('matching-moment');else if(mode==='shared-clock')steps.push('confirm-video-clock');steps.push('output-timespan');}steps.push('optional-overlays');for(const overlay of wizardStepModel.overlayOrder){if(wizardState.decisions.optionalOverlays.includes(overlay))steps.push(wizardStepModel.overlaySteps[overlay]);}return [...steps,...wizardStepModel.tail];}"
