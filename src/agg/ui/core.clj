...
(defn page [{:keys [user csrf picker-config tokens members logs-enabled?]}]
  (let [csrf-headers (escape-html
    ...
    (str
     ...
     "function updateVideoTransport(){const current=clampVideoTime(sourceVideo.currentTime),ratio=playableDuration()?current/videoDuration:0,paused=sourceVideo.paused;updateTimelineMode(false);videoTime.textContent=videoClockText(current)+' / '+videoClockText(videoDuration);videoPlayhead.style.left=(ratio*100)+'%';videoTimeline.setAttribute('aria-valuenow',String(current));videoTimeline.setAttribute('aria-valuetext',videoClockText(current));videoPlayPause.setAttribute('aria-label',paused?'Play video':'Pause video');videoPlayPauseSymbol.textContent=paused?'▶':'⏸';}"
     ...
     "function finishedManualSynchronization(){return hasSourceVideo()&&wizardState.activeRoute==='finished-video'&&selectedSynchronizationMode()==='manual-anchor';}"
