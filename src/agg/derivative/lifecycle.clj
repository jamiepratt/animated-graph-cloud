...
(def active-states #{:queued :running :cancellation-requested})
(def terminal-states #{:succeeded :failed :cancelled :expired :revoked})

(def ^:private profile-version
  (get-in contract/contract-v1 [:profile :version]))

(def ^:private environment
  (:environment contract/contract-v1))
...
(defn- invalid-transition! [job event]
  (throw
   (errors/raise! "Invalid private-preview transition"
...
(defn transition
  "Applies one pure production private-preview lifecycle event."
...
(defn public-resource
  "Projects a preparation onto its bounded browser-visible contract."
  [job]
  (let [id (:id job)]
    (cond-> {:id id
