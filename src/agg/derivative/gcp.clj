...
(defn- eligible-cache-job
  ...
  (when (.exists snapshot)
    (let [data (.getData snapshot)
      ...
      (when (and expires-at
                 (= "production" (get data "environment"))
                 (not (.isBefore
                       expires-at
...
(defrecord FirestorePreparationService
  ...
  (owns-preparation? [_ job-id subject]
    (let [job
          (snapshot-job
           (await! (.get (.document
                          (.collection firestore "production-derivative-preparation-jobs-v1")
                          job-id))))]
  ...
  (submit-preparation! [_ idempotency-key raw-request]
    ...
    (let [profile-version (get-in contract/contract-v1 [:profile :version])
          ...
          jobs (.collection firestore "production-derivative-preparation-jobs-v1")
          idempotency
          (.collection firestore "production-derivative-preparation-idempotency-v1")
          cache (.collection firestore "production-derivative-preview-cache-v1")
          orchestration
          (.collection firestore "production-derivative-active-jobs-v1")
          owner-key (sha256 (:subject request))
          idempotency-ref
          (.document idempotency
                     (keys/idempotency-key fingerprint-secret environment
                                           (:subject request)
                                           idempotency-key))
        ...
        (let [launch
          ...
          (if-let [error (:error launch)]
            (let [admission-ref
                  (.document
                   (.collection
                    firestore "production-derivative-active-jobs-v1")
                   "admission")]
              (transaction!
               firestore
               (fn [transaction]
  ...
  (reconcile-preparations! [this]
    (let [jobs
          (->> (await! (.get (.collection
                              firestore "production-derivative-preparation-jobs-v1")))
               .getDocuments
               (keep snapshot-job)
               vec)
          repaired (atom 0)
  ...
  (cancel-member-jobs! [this {:keys [subject] :as cleanup-identity}]
    (let [jobs (.collection firestore "production-derivative-preparation-jobs-v1")
          candidates
