...
(def playback-range-worker
  (str
   "const playbackPath=/^\\/v1\\/drive\\/playback\\/[0-9A-Fa-f-]{36}$/;"
...
(defn page [{:keys [user csrf folder-id]}]
  (let [csrf-json (json/write-str csrf)
    ...
    (str
     ...
     "const state={viewerEmail,fixedFolderId,rangeAdapter:'pending',listing:null,selected:null,sourcesById:new Map(),analysis:null,analysisFailure:null,support:null,playback:{heuristic:null,actual:'not_attempted'},timing:null,session:null,rangeProbe:null,media:{events:[],buffered:[],currentTime:0,duration:null,error:false}};let selectionGeneration=0;"
     ...
     "function renderPreparationDebug(){prepDebug.textContent=JSON.stringify({viewerEmail:state.viewerEmail,folderId:state.fixedFolderId,rangeAdapter:state.rangeAdapter,listingMode:state.listing&&state.listing.listingMode||null,selected:state.selected,analysis:state.analysis,analysisFailure:state.analysisFailure,support:state.support,playback:state.playback,timing:state.timing&&{status:state.timing.status,ambiguous:state.timing.ambiguous,recommendedIndex:state.timing.recommendedIndex,candidates:state.timing.candidates,durationSeconds:state.timing.durationSeconds},session:state.session&&{playbackUrl:state.session.playbackUrl,contentType:state.session.contentType,size:state.session.size}},null,2);}"
