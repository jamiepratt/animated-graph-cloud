...
(defn page [{:keys [user csrf folder-id]}]
  (let [csrf-json (json/write-str csrf)
    ...
    (str
     ...
     "const state={viewerEmail,fixedFolderId,listing:null,selection:null};"
     ...
     "function activeSelection(){return state.selection;}function isCurrent(selection){return state.selection===selection;}function newSelection(source){return {source,analysis:null,support:null,timing:null,session:null,rangeProbe:null,media:{events:[],buffered:[],currentTime:0,duration:null,error:false}};}"
     "function renderPreparationDebug(){const selection=activeSelection();prepDebug.textContent=JSON.stringify({viewerEmail:state.viewerEmail,folderId:state.fixedFolderId,listingMode:state.listing&&state.listing.listingMode||null,selected:selection&&selection.source,analysis:selection&&selection.analysis,support:selection&&selection.support,timing:selection&&selection.timing&&{status:selection.timing.status,ambiguous:selection.timing.ambiguous,recommendedIndex:selection.timing.recommendedIndex,candidates:selection.timing.candidates,durationSeconds:selection.timing.durationSeconds},session:selection&&selection.session&&{playbackUrl:selection.session.playbackUrl,contentType:selection.session.contentType,size:selection.session.size}},null,2);}"
     "function renderRangeDebug(){const selection=activeSelection();rangeDebug.textContent=JSON.stringify({rangeProbe:selection&&selection.rangeProbe,media:selection&&selection.media||{events:[],buffered:[],currentTime:0,duration:null,error:false}},null,2);}"
     "function updateTiming(){const selection=activeSelection(),inspection=selection&&selection.timing,source=selection&&selection.source,start=parseRecordingStart(inspection),duration=source&&source.durationSeconds||inspection&&inspection.durationSeconds||null,end=start?deriveRecordingEnd(start,duration):null;setText(timingStart,start||'Unknown');setText(timingEnd,end||'Unknown');if(!inspection){setText(timingState,'Waiting for source selection');setText(timingConfidence,'-');return;}if(inspection.ambiguous){setText(timingState,'Ambiguous');setText(timingConfidence,'Multiple credible recording-clock candidates');}else if(start){setText(timingState,'Candidate');setText(timingConfidence,'Recommended container candidate');}else if(Number.isFinite(duration)&&duration>0){setText(timingState,'Duration only');setText(timingConfidence,'Recording start unknown');}else{setText(timingState,'Unknown');setText(timingConfidence,'No trustworthy evidence found');}}"
     "function updateSummary(){const selection=activeSelection(),source=selection&&selection.source;setText(summaryListing,state.listing&&state.listing.listingMode||'Unknown');setText(summaryFile,source&&source.fileName||'No source selected');setText(summaryMime,source&&source.mimeType||'-');setText(summaryDuration,formatDuration(source&&source.durationSeconds));setText(summarySize,formatFrameSize(source));updateTiming();renderPreparationDebug();renderRangeDebug();}"
     "function rememberMedia(eventName){const selection=activeSelection();if(!selection)return;const media=selection.media;media.events=[...media.events.slice(-11),{event:eventName,currentTime:Number((player.currentTime||0).toFixed(3)),duration:Number.isFinite(player.duration)?Number(player.duration.toFixed(3)):null,buffered:bufferedRanges()}];media.buffered=bufferedRanges();media.currentTime=Number((player.currentTime||0).toFixed(3));media.duration=Number.isFinite(player.duration)?Number(player.duration.toFixed(3)):null;renderRangeDebug();}"
     "['loadedmetadata','progress','seeking','seeked','waiting','canplay','timeupdate','error'].forEach(name=>player.addEventListener(name,()=>{const selection=activeSelection();if(selection&&name==='error')selection.media.error=true;rememberMedia(name);}));"
     "async function probeRange(selection,url){const response=await fetch(url,{credentials:'same-origin',headers:{Range:'bytes=0-4095'}});await response.arrayBuffer();if(!isCurrent(selection))return false;selection.rangeProbe={status:response.status,contentRange:response.headers.get('Content-Range'),contentLength:response.headers.get('Content-Length'),contentType:response.headers.get('Content-Type')};renderRangeDebug();return true;}"
     "async function selectSource(source){const selection=newSelection(source);state.selection=selection;player.removeAttribute('src');player.load();selectedTitle.textContent=source.fileName;showStatus(playerStatus,'Inspecting browser playback support and recording clock…');updateSummary();try{const [analysisResponse,timingResponse]=await Promise.all([fetch('/v1/drive/playback-analyses',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json','X-CSRF-Token':csrf},body:JSON.stringify({fileId:source.fileId})}),fetch('/v1/drive/recording-clock-inspections',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json','X-CSRF-Token':csrf},body:JSON.stringify({fileId:source.fileId})})]);if(!isCurrent(selection))return;if(!analysisResponse.ok)throw new Error('Playback analysis failed');selection.analysis=await analysisResponse.json();if(!isCurrent(selection))return;selection.support=await browserSupport(selection.analysis&&selection.analysis.evidence);if(!isCurrent(selection))return;if(timingResponse.ok){selection.timing=await timingResponse.json();if(!isCurrent(selection))return;}if(!selection.support.supported){showStatus(playerStatus,'This source stays listed, but the browser could not prove direct playback support. See bounded evidence below.','error');updateSummary();return;}const sessionResponse=await fetch('/v1/drive/playback-sessions',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json','X-CSRF-Token':csrf},body:JSON.stringify({fileId:source.fileId})});if(!isCurrent(selection))return;if(!sessionResponse.ok)throw new Error('Playback session failed');selection.session=await sessionResponse.json();if(!isCurrent(selection))return;if(!selection.session||typeof selection.session.playbackUrl!=='string'||!selection.session.playbackUrl.startsWith('/v1/drive/playback/'))throw new Error('Playback session returned an invalid private playback URL');if(!await probeRange(selection,selection.session.playbackUrl))return;if(!isCurrent(selection))return;player.src=selection.session.playbackUrl;player.load();showStatus(playerStatus,'Private playback prepared. Seek the player and watch the range debug panel update.','success');updateSummary();}catch(error){if(!isCurrent(selection))return;showStatus(playerStatus,error&&error.message?error.message:'Playback preparation failed','error');renderPreparationDebug();renderRangeDebug();}}"
