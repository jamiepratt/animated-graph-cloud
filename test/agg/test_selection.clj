...
(defn- impact-rule [path]
  (cond
    ...
    (str/starts-with? path "infra/proto/")
    {:tests #{'agg.deploy-workflow-test
              'agg.proto-release-test
              'agg.release-config-test}
     :reason (str "proto infrastructure contract for " path)}
    ...
    (or (= path "CHANGELOG.md")
        (= path "docs/proto/CHANGELOG.md")
        (= path "resources/agg/main-version.edn")
        (= path "resources/agg/proto-version.edn"))
    {:tests #{'agg.release-test
              'agg.proto-release-test
              'agg.release-config-test}
