...
(deftest worker-publication-uses-the-configured-immutable-asset-store
  (let [calls (atom [])
        ...
        publication {:job-id "job-id"
                     :attempt 1
                     :object-key "production/derivative-previews/v1/final.mp4"}]
    (is (= {:generation 42
            :size 1024
            :content-type "video/mp4"
            :profile-version "h264-aac-1080p25-v1"}
    ...
    (is (true?
         (gcp/delete-derivative!
          service {:object-key "production/derivative-previews/v1/final.mp4"
                   :generation 42})))
    (is (= [[:publish publication]
            [:delete {:object-key "production/derivative-previews/v1/final.mp4"
                      :generation 42}]]
           @calls))))
...
(deftest firestore-admission-dispatch-and-cancellation-are-attempt-exact
  (if-let [host (System/getenv "FIRESTORE_EMULATOR_HOST")]
    (let [firestore (-> (FirestoreOptions/newBuilder)
      ...
      (try
        (doseq [collection ["production-derivative-preparation-jobs-v1"
                            "production-derivative-preparation-idempotency-v1"
                            "production-derivative-preview-cache-v1"
                            "production-derivative-active-jobs-v1"]]
        ...
        (let [results
          ...
          (let [attempt
            ...
            (is (re-matches #"production/derivative-previews/v1/[0-9a-f]{64}\.mp4"
          ...
          (let [acknowledged
                ...
                (derivative/acknowledge-preparation-cancellation!
                 service job-id 1)]
            (is (= {:state "cancelled"}
                   (select-keys acknowledged [:state :cancellationLagMs])))
    ...
    (let [firestore (-> (FirestoreOptions/newBuilder)
      ...
      (try
        ...
        (let [job-id
          ...
          (is (not (str/includes?
                    public-text "production/derivative-previews/v1/"))))
        (finally
          (.close firestore))))
    (is true "Firestore emulator test is run by the targeted emulator command")))
