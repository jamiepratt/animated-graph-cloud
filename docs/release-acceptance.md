...
| Homepage video playlist | The signed-out homepage loads playlist `PLIIYTIXqGbuE` in owner-defined order on desktop and mobile; a newly added video appears without deployment; selecting and opening each item preserves both `v` and `list`; blocking YouTube demonstrates the full-playlist fallback without breaking the page. |
