...
| OAuth brand verification | Alpha Compose brand published in the production Google project with exact homepage, privacy/terms URLs, authorized domain, exact combined login callbacks for `https://alphacompose.com/v1/auth/login/callback` and `https://proto.alphacompose.com/v1/auth/login/callback`, exactly `openid email profile drive.file` for the main app flow, and exactly `openid email profile drive.file https://www.googleapis.com/auth/drive.readonly` for the proto flow. Confirm the old Drive callback was removed after deployment. |
