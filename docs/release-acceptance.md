...
| Private-preview lifecycle | With explicit owner approval for processing allowance, use a clean signed-in page and confirm the scoped range-adapter worker is active and controlling before prepared attachment. Prepare one browser-incompatible source, confirm the original video is unchanged, playback is owner-bound, expiry is 24 hours, and submission through terminal state is discoverable by the response `X-Request-Id` in admin logs. Verify one real nonzero media request returns a truthful HTTP 206 `Content-Range`. Do not perform this costed check during repository acceptance. |
...
| Homepage video playlist | The signed-out homepage loads playlist `PLIIYTIXqGbuE` in owner-defined order with four, three, two, or one rich cards as space permits; a newly added or reordered video appears within 15 minutes without deployment; selecting and opening each item preserves both `v` and `list`; automatic one-card rotation, wrap, manual stop, reduced motion, hover/focus/touch/scroll/hidden/playback pauses, and all independent player/metadata failure combinations work without page overflow. Confirm the deployed key is project-owned, restricted only to `youtube.googleapis.com`, and mounted only on `agg-api`. |
